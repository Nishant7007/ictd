...
def getAnomaliesBySameMonthMonthly(commodity, folderToOpen, filename, startDate, isOriginal = False):
#	print(startDate)
	mads = []
	medians = []
	...
	files = [f for f in os.listdir(folderToOpen) if f.endswith(name) and (f!=filename)]
	for file in files:
...
def getAnomaliesBySameMonth():
	commodityList = ['ONION', 'POTATO', 'MUSTARD OIL', 'MASUR DAL', 'RICE', 'TOMATO', 'GREEN GRAM DAL (MOONG DAL)', 'SUGAR']
	...
	for commodity in commodityList: 
		...
		for file in files:
			try:
				...
				fileToSave = os.path.join(folderToOpen.replace('Original','Anomaly/SameMonth'), file)
				finalDf = pd.DataFrame(columns=['DATE', 'ANOMALY'])
				for date in dates[:-1]:
					d = date.strftime('%Y-%m-%d')
					temp = getAnomaliesBySameMonthMonthly(commodity, folderToOpen, file, d)
					finalDf = finalDf.append(temp, ignore_index=True)
				date = dates[-1]
				d = date.strftime('%Y-%m-%d')
				temp = getAnomaliesBySameMonthMonthly(commodity, folderToOpen, file, d, False)
				finalDf = finalDf.append(temp, ignore_index=True)
